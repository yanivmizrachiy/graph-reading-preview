...
for (const path of ['demo.html','דפים.html','CLAUDE-CODE-HANDOFF.md','הסתברות','רמה 1','רמה 2','רמה 3','רמה 4','רמה 5']) {
...
for (const forbidden of ['id="zoomMode"','class="zoom-control"','class="mode-switch"','id="colorMode"','id="bwMode"','id="viewerModeLabel"','id="viewerHelp"','id="sourceBadge"','class="viewer-head"','class="intro"','class="legend"','דפדוף נוח','<b>A4</b>','חוברת דיגיטלית ·','<select']) forbidText(index, forbidden, 'index.html');

// One unified toolbar only: page navigation + print/download/fullscreen.
forbidText(index, 'class="action-bar"', 'index.html');
...
for (const forbidden of ['usesHtmlBook','navigator.userAgent','maxTouchPoints','pdfViewUrl','previewUrl',"params.get('mode')","$('colorMode')","$('bwMode')",'worksheet-bw.css']) forbidText(js, forbidden, 'graph-reading-workbook.js');
...
if (masterPageCount !== meta.pageCount) fail(`מאגר-מלא.html: ${masterPageCount} .a4-page elements, manifest expects ${meta.pageCount}`);
...
if (indexRelease && jsRelease && indexRelease !== jsRelease) fail(`release mismatch: index=${indexRelease}, js=${jsRelease}`);
if (indexRelease && meta.release !== indexRelease) fail(`release mismatch: meta=${meta.release}, index=${indexRelease}`);
if (indexRelease) {
...
for (const kind of ['color', 'bw']) {
  ...
  if (actualBytes !== file.bytes) fail(`${file.path}: bytes=${actualBytes}, manifest=${file.bytes}`);
